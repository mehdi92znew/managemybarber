...
const getPaymentStatusClass = (status) => {
    switch (status) {
        ...
        case 'semi-paid': return 'bg-amber-500/10 text-amber-500 border-amber-500/20';
        case 'unpaid': 
        case 'pending':
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-6 space-y-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
            <!-- Filters -->
            <div class="grid grid-cols-1 md:grid-cols-6 gap-4 bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-white/5 premium-shadow items-end">
                <div class="space-y-2">
                    <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 px-1">{{ __('barber') }}</label>
                    <select v-model="filterForm.barber_id" class="w-full rounded-2xl border-slate-100 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-bold text-slate-900 dark:text-white focus:border-amber-500 focus:ring-0 transition-all">
                        <option value="">{{ __('all_barbers') }}</option>
                        <option v-for="b in barbers" :key="b.id" :value="b.id">{{ b.name }}</option>
                    </select>
                </div>
                <div class="space-y-2">
                    <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 px-1">{{ __('status') }}</label>
                    <select v-model="filterForm.status" class="w-full rounded-2xl border-slate-100 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-bold text-slate-900 dark:text-white focus:border-amber-500 focus:ring-0 transition-all">
                ...
                <div class="space-y-2">
                    <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 px-1">{{ __('payment_status') }}</label>
                    <select v-model="filterForm.payment_status" class="w-full rounded-2xl border-slate-100 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm font-bold text-slate-900 dark:text-white focus:border-amber-500 focus:ring-0 transition-all">
                        <option value="">{{ __('all_payment_statuses') }}</option>
                ...
                <button @click="clearFilters" class="w-full py-3 rounded-2xl bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all">
                    {{ __('clear_filters') }}
                </button>
            </div>
                ...
                <table class="hidden lg:table min-w-full divide-y divide-slate-100 dark:divide-white/5">
                    <thead class="bg-slate-50/50 dark:bg-black/20">
                        <tr>
                            ...
                            <th class="px-8 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ __('payment_status') }}</th>
                            <th class="px-8 py-5 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ __('price') }}</th>
                            <th class="px-8 py-5 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ __('actions') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                        <tr v-for="appt in appointments.data" :key="appt.id" class="group hover:bg-slate-50/80 dark:hover:bg-white/5 transition-all">
                            <td class="px-8 py-6">
                                <span class="text-sm font-bold text-slate-600 dark:text-slate-400">{{ formatDateTime(appt.start_time) }}</span>
                            </td>
                            <td class="px-8 py-6">
                                <div class="flex flex-col">
                                    <span class="text-sm font-black text-slate-900 dark:text-white">{{ appt.customer?.name || __('walk_in') }}</span>
                                    <span class="text-[10px] font-bold text-slate-400">{{ appt.customer?.phone || '-' }}</span>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-3">
                                    <div class="h-8 w-8 rounded-lg bg-amber-500/10 text-amber-500 flex items-center justify-center text-[10px] font-black">
                                        {{ appt.barber?.name.charAt(0) }}
                                    </div>
                                    <span class="text-sm font-bold text-slate-700 dark:text-slate-300">{{ appt.barber?.name }}</span>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                                <span :class="['px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border', getStatusClass(appt.status)]">
                                    {{ __(appt.status) }}
                                </span>
                            </td>
                            <td class="px-8 py-6">
                                <span :class="['px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border', getPaymentStatusClass(appt.payment_status)]">
                                    {{ __(appt.payment_status) }}
                                </span>
                            </td>
                            <td class="px-8 py-6 text-right">
                                <span class="font-black text-slate-900 dark:text-white">{{ formatCurrency(appt.total_price) }}</span>
                            </td>
                            <td class="px-8 py-6 text-right">
                                <button 
            ...
            <BookingModal 
                ...
                @close="isEditModalOpen = false; selectedAppt = null"
