...
<template>
    <div class="space-y-6">
            ...
            <table class="min-w-full divide-y divide-slate-100 dark:divide-white/5">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                    <tr v-for="barber in barbers" :key="barber.id" class="group hover:bg-slate-50/80 dark:hover:bg-white/5 transition-all">
                        ...
                        <td class="px-8 py-6">
                            <span class="px-4 py-2 rounded-xl bg-rose-500/10 text-rose-600 dark:text-rose-400 text-xs font-black uppercase tracking-widest">
                        ...
                        <td class="px-8 py-6 text-right">
                            <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button @click="openEditModal(barber)" class="p-2.5 rounded-xl bg-slate-100 hover:bg-rose-500 hover:text-white dark:bg-white/5 transition-all">
        ...
        <div v-else class="grid grid-cols-1 gap-4 pb-20">
            <div v-for="stylist in stylists" :key="stylist.id" class="p-6 rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 premium-shadow">
                <div class="flex items-center gap-4 mb-6">
                     <div class="h-14 w-14 rounded-[1.25rem] bg-rose-500/10 text-rose-500 flex items-center justify-center font-black text-2xl">
                        {{ stylist.name.charAt(0) }}
                    </div>
                    <div>
                        <h3 class="font-black text-slate-900 dark:text-white uppercase tracking-tight">{{ stylist.name }}</h3>
                        <p class="text-xs text-slate-500 font-medium">{{ stylist.email }}</p>
                    </div>
                </div>
