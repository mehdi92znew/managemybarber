...
const formatShortDate = (date) => {
    ...
    const dateLocale =
        currentLocale.value === "fr" || currentLocale.value === "ar"
            ? "fr-FR"
            : "en-US";
...
<template>
    <div class="space-y-6">
        <!-- Header Section -->
        <div
            class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-white/5 premium-shadow"
        >
            <div>
                <h2
        ...
        <!-- Filters Section -->
        <div
            class="grid grid-cols-1 md:grid-cols-4 gap-4 bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-white/5 premium-shadow"
        >
            <div class="space-y-2">
                <label
            ...
            <div class="flex items-end">
                <button
                    @click="clearFilters"
                    class="w-full py-3 rounded-2xl bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-white/10 transition-all"
                >
                    {{ __("clear_filters") }}
                </button>
            </div>
                    ...
                    >
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-3">
                                <div
                                    class="p-2 rounded-xl bg-slate-100 dark:bg-white/5 text-slate-500"
                                >
                                    <svg
                                        class="w-4 h-4"
                                        fill="none"
                                ...
                                <span
                                    class="text-sm font-bold text-slate-900 dark:text-white capitalize"
                                    >{{ formatDate(payout.date) }}</span
                                >
                            </div>
                        </td>
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-3">
                                ...
                                <span
                                    ...
                                    >{{ payout.barber.name }}</span
                                >
                            </div>
                        </td>
                        <td class="px-8 py-6">
                            <p
                                class="text-sm font-medium text-slate-500 italic max-w-xs truncate"
                            >
                                {{ payout.note || "-" }}
                            </p>
                        </td>
                        <td class="px-8 py-6 text-right">
                            <span
                                ...
                                >{{ formatCurrency(payout.amount) }}</span
                            >
                        </td>
                        <td class="px-8 py-6 text-right">
                            <button
                                @click="deletePayout(payout.id)"
                                class="p-2.5 rounded-xl bg-slate-100/50 hover:bg-red-500 hover:text-white dark:bg-white/5 transition-all opacity-0 group-hover:opacity-100"
                            >
                                <svg
                                    class="w-4 h-4"
                                    fill="none"
        ...
        <div v-else class="grid grid-cols-1 gap-4 pb-20">
            <div
                v-for="payout in payouts.data"
                :key="payout.id"
                class="p-5 rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 premium-shadow"
            >
                <div class="flex justify-between items-start mb-4">
                    ...
                    <div class="text-right">
                        ...
                        >
                        <span
                            class="text-sm font-black text-emerald-500 underline decoration-emerald-500/30 decoration-2 underline-offset-4"
                            >{{ formatCurrency(payout.amount) }}</span
                        >
                    </div>
                </div>
                ...
                <div
                    class="p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 mb-4"
                >
                    <p class="text-xs font-medium text-slate-500 italic">
                        {{ payout.note || __("no_note") }}
                    </p>
                </div>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <span
                            ...
                            >{{ formatShortDate(payout.date) }}</span
                        >
                    </div>
                    <button
                        @click="deletePayout(payout.id)"
                        class="px-4 py-2 rounded-xl bg-red-50 text-red-600 text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all"
                    >
                        {{ __("delete") }}
                    </button>
                </div>
        ...
        >
            ...
            <p class="text-sm text-slate-500 mt-2 font-medium">
                {{ __("add_payouts_to_track_expenses") }}
            </p>
            <PrimaryButton @click="showAddModal = true" class="mt-8">{{
                __("add_your_first_payout")
            }}</PrimaryButton>
        </div>
