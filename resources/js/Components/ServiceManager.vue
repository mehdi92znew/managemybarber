    ...
    <div class="space-y-6">
                    ...
                    <tr v-for="service in services.data" :key="service.id" class="group hover:bg-slate-50/80 dark:hover:bg-white/5 transition-all">
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <span v-if="service.is_extra" class="text-[9px] font-black uppercase tracking-widest text-rose-500">{{ __('extra_service') }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="px-8 py-6">
                        ...
                        <td class="px-8 py-6 text-right">
                            <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button @click="openEditModal(service)" class="p-2 rounded-lg bg-slate-100 hover:bg-rose-500 hover:text-white dark:bg-white/5 transition-all">
        ...
        <div v-else class="grid grid-cols-1 gap-4 pb-20">
            <div v-for="service in services.data" :key="service.id" class="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 premium-shadow group">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-2 mt-1">
                                ...
                                <span v-if="service.is_extra" class="h-1 w-1 rounded-full bg-rose-500"></span>
                                <span v-if="service.is_extra" class="text-[9px] font-black uppercase tracking-widest text-rose-500">{{ __('extra') }}</span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-3 mt-4">
                    <button @click="openEditModal(service)" class="flex items-center justify-center gap-2 py-3 rounded-xl bg-slate-50 dark:bg-white/5 text-slate-600 dark:text-slate-400 font-bold text-xs hover:bg-rose-500 hover:text-white transition-all">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                        {{ __('edit') }}
                    </button>
        ...
        <div v-if="services.data.length === 0" class="flex flex-col items-center justify-center py-20 px-6 text-center bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-white/5">
             <div class="p-6 rounded-3xl bg-slate-50 dark:bg-white/5 text-slate-300 dark:text-slate-700 mb-6">
                <svg class="w-16 h-16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-7.714 2.143L11 21l-2.286-6.857L1 12l7.714-2.143L11 3z" /></svg>
             </div>
        ...
        <button 
            ...
            class="fixed bottom-8 right-6 z-40 h-14 w-14 rounded-2xl bg-rose-500 text-slate-900 shadow-2xl shadow-rose-500/40 flex items-center justify-center active:scale-90 transition-all border-4 border-white dark:border-slate-900"
        >
        ...
        <Modal :show="isModalOpen" @close="closeModal">
            <div class="p-6 sm:p-10 bg-white dark:bg-slate-900">
                ...
                <div class="space-y-6">
                    ...
                    <label class="flex items-center gap-4 p-5 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 cursor-pointer group transition-all" :class="form.is_extra ? 'border-rose-500/30 bg-rose-500/5' : ''">
                        <div class="relative w-10 h-6">
                             <input
                                ...
                                class="sr-only"
                            />
                            <div class="w-10 h-6 bg-slate-200 dark:bg-slate-700 rounded-full transition-colors" :class="form.is_extra ? 'bg-rose-500' : ''"></div>
